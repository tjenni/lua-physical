...
In principle this library is standalone, but it is compatible with the |siunitx| package. Calculation results done in \LuaLaTeX{} can be printed directly by calling the |tosiunitx()| method.


\section{Usage}

Given the basic preamble, units can be used in lua code directly. By convention, all units have an underscore in front of them, i.e. meter is |_m|, second is |_s|. All available units are listed in chapter \ref{ch:Supported Units}. The results of calculations can be printed to \LuaLaTeX{} by using the macro |\q{}|.
...
Most of the units transform linearly to base units. Exceptions are the unit degree Celsius |_degC| and degree Fahrenheit |_degF|. These units are ambigous and can be interpreted as temperature differences or as an absolute temperatures. In the latter case, the conversion to base units is not a linear, but an affine transformation. This is because degree Celsius and degree Fahrenheit scales have their zero points at different temperatures compared to the unit Kelvin. 

By default |_degC| and |_degF| units are standing for temperature differences. If one wants to have it converted absolutely, it has to be done adding / subtracting the quantities |_degC_0| and |_degF_0|. 
...
Instead of printing always the uncertainties, one can use the uncertainty calculation to provide significant digits.
...
The |lua-physical| library has 9 base quantities. These are the seven basis units or basis quantities of the SI system and in addition the base quantity of information |_bit| and of currency |_EUR|. All other quantities are derived from these.
...
\newcommand{\printconstant}[2]{
  ...
  |#1| &
  \mbox{|#2|}\index{\directlua{tex.print(#1.unit.name)} \texttt{\directlua{tex.print( strtoidx("#1") )}}}\\
}
...
\newcommand{\printunit}[3][]{
  ...
  |#2| &
  \mbox{|#3|}\index{\directlua{tex.print(#2.unit.name)} \texttt{\directlua{tex.print( strtoidx("#2") )}}} \\
}
...
There are a few units with dimension $\mathrm{1}$.

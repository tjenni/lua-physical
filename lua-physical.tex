...
\begin{quote}
  The constuctor of the |Quantity| class. It takes an optional |quantity| or |number| as argument |q|. If it is given, the new quantity is a copy of |q|. If no argument ist given, a quantity |_1| is created.

\begin{example}[listing only]{Create some quantities.}
myNumber = Q(42)
myLength = Q(73*_m)
\end{example}
...
\begin{example}[listing only]{Define some base quantities.}
...
\begin{example}[listing only]{Create new quantities from others.}
...
\begin{example}[listing only]{Define a new prefix}
...
\begin{example}[listing only]{Add prefixes to some quantites.}

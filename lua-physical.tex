...
In the following problem, listing \ref{lst:degree to kelvin} , the task is to convert a temperature given in the unit degree Celsius to Kelvin. 
...
A thermometer shows $\q{theta}$. Convert this quantity to Kelvin temperature. 
$$
  T = \q{theta} + \q{_degC_0:to()}
  =  \q{T}
$$
\end{lstlisting}
...
$$
  T = \q{theta:to(_K)} + \q{_degC_0:to(_K)}
  =  \q{(theta + _degC_0):to(_K)}
$$
\endleftbar
